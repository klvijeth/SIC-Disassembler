...
void headerConverter(string objLine)
{
    ...
    if(header[0]=="H")
    {
        fp_sic<<header[3]<<"    "<<header[1]<<"    "<<"START   "<<header[3]<<endl;
...
void textRecConverter(vector<string> line)
{
    ...
    if(line[0] == "T")
    {
        ...
        for(int i=3;i<line.size();i++)
        {
            ...
            //search address in symtab.
            //check for x
            //put these into the SIC program.
    ...
    else
    {
        ...
        for(int i=1;i<line.size();i+=2)
        {
           ...
           // locctr=stringToHex(line[i+1]);
            
        }

        //Compute Updated LOCCTR.
        //Insert RESW, RESB
        //Set the flag to handle in text record section.
    }
}
