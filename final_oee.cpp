...
/*
    Structure for Declaration, Opcode and Symbol Tables.
*/
struct optab
{
    string opcode;
...
/*
    Global Variables.
*/
int programLength = 0x00,rflag=0,tempAddress=0x00;
long long locctr = 0x00,firstAddr = 0x00;
...
/*
    ...
    Description: Converts ascii values to character string for BYTE instruction or char type.
    Arguments: string of character byte object code.
    Return Type: string of characters.
*/
...
/*
    Function: dectabGeneration()
    Description: Creates Declaration Table from Declaration records of RESB,RESW,BYTE,WORD. .
    Arguments: None
    Return Type: Void
*/
...
/*
    ...
    Arguments: Vector of strings header record.
    Return Type: void
*/

void headerConverter(vector<string> header)
{
    fp_sic<<std::hex<<stringToHex(header[2])<<"\t"<<header[1]<<"\t"<<"START   "<<stringToHex(header[2])<<endl;
...
/*
    Function: textRecConverter()
    Description: Converts and loads the Object Program Text Records to SIC program.
    Arguments: vector of strings Text record or Reserve declaration record.
    Return Type: Void
*/
        ...
        for(int i=3;i<line.size();i++)
        {
            ...
            for(dectab d : dectable)
            {
                if(d.address==locctr)
                {
                    ...
                    {
                        fp_sic<<"BYTE\t"<<"X'"<<hex<<oc<<"'"<<endl;
                        locctr+=(line[i].length()/2);
                    }
                    else if(d.type=="WORD")
                    {
                        cout<<"its word\t"<<dec<<oc<<endl;
                        fp_sic<<"WORD\t"<<dec<<oc<<endl;
                        locctr+=3;
                    }
                    flag2=1;
                    break;
...
/*
    Function: endRecConverter()
    Description: Converts and loads end record from object program to SIC program. .
    Arguments: vector of strings of End Record.
    Return Type: Void
*/
